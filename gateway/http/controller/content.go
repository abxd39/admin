package controller

import (
	"admin/gateway/log"
	"admin/gateway/rpc"
	Err "admin/proto/common"
	proto "admin/proto/rpc"
	"fmt"
	"net/http"

	"github.com/gin-gonic/gin"
)

type ContentManage struct{}

func (c *ContentManage) Router(r *gin.Engine) {
	cont := r.Group("/content")

	{
		cont.POST("/b", c.Banner)
		cont.POST("/addlink", c.AddFriendlyLink)
		cont.GET("/linklist", c.GetFriendlyLink)
	}
}

func (cm *ContentManage) Banner(c *gin.Context) {
	ret := Err.NewPublciError()
	defer func() {
		c.JSON(http.StatusOK, ret.GetResult())
	}()
	return
}

func (cm *ContentManage) AddFriendlyLink(c *gin.Context) {
	ret := Err.NewPublciError()
	defer func() {
		c.JSON(http.StatusOK, ret.GetResult())
	}()
	req := struct {
		WebName   string `form:"web_name" json:"web_name" binding:"required"`
		LinkName  string `form:"link_name" json:"link_name" binding:"required"`
		Aorder    int32  `form:"order" json:"order" binding:"required"`
		LinkState int32  `form:"link_state" json:"link_state" binding:"required"`
	}{}
	err := c.ShouldBind(&req)
	if err != nil {
		log.Log.Errorf(err.Error())
		ret.SetErrCode(Err.ERRCODE_PARAM, err.Error())
		return
	}
	rsp, err := rpc.InnerService.BackstageSevice.CallAddFriendlyLink(&proto.AddFriendlyLinkRequest{
		WebName:   req.WebName,
		LinkName:  req.LinkName,
		Aorder:    req.Aorder,
		LinkState: req.LinkState,
	})
	if err != nil {
		log.Log.Errorf(err.Error())
		return
	}
	ret.SetErrCode(rsp.Code)
	return
}

func (cm *ContentManage) GetFriendlyLink(c *gin.Context) {
	ret := Err.NewPublciError()
	defer func() {
		c.JSON(http.StatusOK, ret.GetResult())
	}()

	req := struct {
		Page  int32 `form:"page" json:"page" binding:"required"`
		Count int32 `form:"count" json:"count" `
	}{}
	err := c.ShouldBind(&req)
	if err != nil {
		log.Log.Errorf(err.Error())
		ret.SetErrCode(Err.ERRCODE_PARAM, err.Error())
		return
	}
	fmt.Println("2121211111111111111111")
	fmt.Println(req)
	rsp, err := rpc.InnerService.BackstageSevice.CallGetFriendlyLink(&proto.FriendlyLinkRequest{})
	if err != nil {
		log.Log.Errorf(err.Error())
		return
	}
	ret.SetErrCode(rsp.Code)
	ret.SetDataSection("list", rsp.Friend)
	return
}
