...
//根据货币id 获取所有用户的 余额和冻结数量
func (this *UserCurrency) GetUserCurrencyBalanceAndFree(tid int) (balance, free int64, err error) {
	engine := utils.Engine_currency
	result, err := engine.Where("token_id=?", tid).SumsInt(&UserCurrency{}, "balance", "freeze")
	if err != nil {
		return 0, 0, err
	}
...
func (this *UserCurrency) CurrencyBalanceNew(page, rows, status, tid int, search string) (*ModelList, error) {
	engine := utils.Engine_currency
	query := engine.Alias("uc").Desc("uc.uid")
	//query = query.Cols()
	//query = query.Cols()
	...
	if tid != 0 {
		query = query.Where("uc.token_id=?", tid)
	}
	if len(search) != 0 {
		temp := fmt.Sprintf(" concat(IFNULL(uc.`uid`,''),IFNULL(u.`phone`,''),IFNULL(ex.`nick_name`,''),IFNULL(u.`email`,'')) LIKE '%%%s%%'  ", search)
		query = query.Where(temp)
